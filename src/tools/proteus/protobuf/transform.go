...
func (t *Transformer) Transform(p *scanner2.Package) *Package {
	pkg := &Package{
...
func (t *Transformer) transformFunc(pkg *Package, f *scanner2.Func, names nameSet) *RPC {
	var (
		name         = f.Name
		receiverName string
	)
	...
	if f.Receiver != nil {
		n, ok := f.Receiver.(*scanner2.Named)
		if !ok {
			report2.Warn("invalid receiver type for func %s", f.Name)
			return nil
		}
...
func (t *Transformer) transformInputTypes(pkg *Package, types []scanner2.Type, names nameSet, name string) Type {
	return t.transformTypeList(pkg, types, names, name, "Request", "arg")
}

func (t *Transformer) transformOutputTypes(pkg *Package, types []scanner2.Type, names nameSet, name string) Type {
	return t.transformTypeList(pkg, types, names, name, "Response", "result")
}

func (t *Transformer) transformTypeList(pkg *Package, types []scanner2.Type, names nameSet, name, msgNameSuffix, msgFieldPrefix string) Type {
	...
	if len(types) != 1 || types[0].IsRepeated() || !isNamed(types[0]) {
		...
		if _, ok := names[msgName]; ok {
			report2.Warn("tried to register message %s, but there is already a message with that name. RPC %s will not be generated", msgName, name)
			return nil
		}
...
func (t *Transformer) createMessageFromTypes(pkg *Package, name string, types []scanner2.Type, fieldPrefix string) *Message {
	msg := &Message{Name: name}
	for i, typ := range types {
		f := t.transformField(pkg, msg, &scanner2.Field{
...
func (t *Transformer) transformEnum(e *scanner2.Enum) *Enum {
	enum := &Enum{
		Docs:    e.Doc,
		Name:    e.Name,
...
func (t *Transformer) defaultOptionsForScannedEnum(e *scanner2.Enum) (opts Options) {
	opts = Options{
...
func (t *Transformer) transformStruct(pkg *Package, s *scanner2.Struct) *Message {
	msg := &Message{
		Docs:    s.Doc,
		Name:    s.Name,
	...
	for i, f := range s.Fields {
		...
		if field == nil {
			...
			report2.Warn("field %q of struct %q has an invalid type, ignoring field but reserving its position", f.Name, s.Name)
		} else {
...
func (t *Transformer) defaultOptionsForScannedMessage(s *scanner2.Struct) (opts Options) {
	opts = Options{
...
func (t *Transformer) transformField(pkg *Package, msg *Message, field *scanner2.Field, pos int) *Field {
	var (
		typ      Type
...
func (t *Transformer) defaultOptionsForStructField(field *scanner2.Field) Options {
	opts := make(Options)
...
func (t *Transformer) needsNotNullableOption(typ scanner2.Type) bool {
...
func (t *Transformer) transformType(pkg *Package, typ scanner2.Type, msg *Message, field *Field) Type {
	if isError(typ) {
		report2.Error("error type is not supported")
		return nil
	}
	...
	case *scanner2.Basic:
		...
		report2.Warn("basic type %q is not defined in the mappings, ignoring", ty.Name)
	case *scanner2.Map:
		m := NewMap(
...
func (t *Transformer) findMapping(name string) *ProtoType {
	...
	if typ != nil && typ.Warn != "" {
		report2.Warn(typ.Warn, name)
	}

	return typ
}

func removeFirstCtx(types []scanner2.Type) ([]scanner2.Type, bool) {
	if len(types) > 0 {
		first := types[0]
		if isCtx(first) {
			return types[1:], true
...
func removeLastError(types []scanner2.Type) ([]scanner2.Type, bool) {
	if len(types) > 0 {
		ln := len(types)
		last := types[ln-1]
		if isError(last) {
...
func isNamed(typ scanner2.Type) bool {
	_, ok := typ.(*scanner2.Named)
	return ok
}

func isCtx(typ scanner2.Type) bool {
	if ctx, ok := typ.(*scanner2.Named); ok {
		return ctx.Path == "context" && ctx.Name == "Context"
	}
	return false
}

func isError(typ scanner2.Type) bool {
	if err, ok := typ.(*scanner2.Named); ok {
		return err.Path == "" && err.Name == "error"
	}
	return false
}

func isByteSlice(typ scanner2.Type) bool {
	if t, ok := typ.(*scanner2.Basic); ok && typ.IsRepeated() {
		return t.Name == "byte"
	}
	return false
}
...
func (t *Transformer) defaultOptionsForPackage(p *scanner2.Package) Options {
	return Options{
...
func buildNameSet(pkg *scanner2.Package) nameSet {
	l := make(nameSet)
