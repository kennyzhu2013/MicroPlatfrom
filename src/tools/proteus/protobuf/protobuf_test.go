...
func TestIsNullable(t *testing.T) {
	...
	require.True(t, IsNullable(), "Named without Source is nullable")
	SetSource(nullableSource)
	require.True(t, IsNullable(), "Named with nullable Source is nullable")
	SetSource(notNullableSource)
	require.False(t, IsNullable(), "Named with not nullable Source is not nullable")

	// Alias
	typ = NewAlias(nullableType, nullableType)
	require.True(t, IsNullable(), "nullable Type and nullable Underlying without Source makes alias nullable")
	typ = NewAlias(nullableType, notNullableType)
	require.True(t, IsNullable(), "nullable Type and not nullable Underlying without Source makes alias nullable")
	typ = NewAlias(notNullableType, nullableType)
	require.False(t, IsNullable(), "not nullable Type and nullable Underlying without Source makes alias not nullable")
	typ = NewAlias(notNullableType, notNullableType)
	require.False(t, IsNullable(), "not nullable Type and not nullable Underlying without Source makes alias not nullable")

	typ = NewAlias(nullableType, nullableType)
	SetSource(nullableSource)
	require.True(t, IsNullable(), "nullable Type and nullable Underlying with nullable Source makes alias nullable")
	SetSource(notNullableSource)
	require.False(t, IsNullable(), "nullable Type and nullable Underlying with not nullable Source makes alias not nullable")
	typ = NewAlias(notNullableType, nullableType)
	SetSource(nullableSource)
	require.True(t, IsNullable(), "not nullable Type and nullable Underlying with nullable Source makes alias nullable")
	typ = NewAlias(notNullableType, nullableType)
	SetSource(notNullableSource)
	require.False(t, IsNullable(), "not nullable Type and nullable Underlying with not nullable Source makes alias not nullable")

	// Basic
	typ = NewBasic("string")
	require.False(t, IsNullable(), "Basic types are never nullable")

	// Map
	typ = NewMap(notNullableType, nullableType)
	require.True(t, IsNullable(), "map<notNullable>*Nullable is nullable")
	typ = NewMap(notNullableType, notNullableType)
	require.False(t, IsNullable(), "map<notNullable>NotNullable is not nullable")
}
