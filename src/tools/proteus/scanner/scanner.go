...
type Scanner struct {
	...
	files    []string // add to store file names..
}
...
func scanType(typ types.Type) (t Type) {
	...
	case *types.Map:
		...
		val := scanType(u.Elem())
		if val == nil {
			report2.Warn("ignoring map with value type %s", typ.String())
			return nil
		}
		t = NewMap(key, val)
	default:
		report2.Warn("ignoring type %s", typ.String())
		return nil
	}
...
func scanStruct(s *Struct, elem *types.Struct) *Struct {
	for i := 0; i < elem.NumFields(); i++ {
		...
		if s.HasField(v.Name()) {
			report2.Warn("struct %q already has a field %q", s.Name, v.Name())
			continue
		}
		...
		if v.Anonymous() {
			...
			if embedded == nil {
				report2.Warn("field %q with type %q is not a valid embedded type", v.Name(), v.Type())
			} else {
