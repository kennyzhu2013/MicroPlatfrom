...
import (
	"errors"
	"fmt"
	proteus2 "github.com/kennyzhu/go-os/src/tools/proteus"
...
func genProtos(c *cli.Context) error {
	...
	return proteus2.GenerateProtos(proteus2.Options{
		BasePath: path,
		Packages: packages,
	})
}
...
func genRPCServer(c *cli.Context) error {
	return proteus2.GenerateRPCServer(packages)
}
...
func protocExec(protocPath, pkg, outPath, protoFile string) error {
	...
	report2.Info("executing protoc: %s %s %s %s %s", protocPath, protocArgs, genAllGoFastOutOption(outPath), genAllMicroOutOption(outPath), protoFile)
...
func genAllGoFastOutOption(outPath string) string {
	...
	importMappings := protobuf2.DefaultMappings.ToGoOutPath()
