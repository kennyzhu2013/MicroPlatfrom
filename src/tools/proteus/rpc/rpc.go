...
func (g *Generator) Generate(proto *protobuf2.Package, path string) error {
	if len(proto.RPCs) == 0 {
		report2.Warn("no RPCs in the given proto file, not generating anything")
		return nil
	}
...
func (g *Generator) genMethodType(ctx *context, rpc *protobuf2.RPC) *ast.FuncType {
	var in, out string
...
func (g *Generator) genMethodCall(ctx *context, rpc *protobuf2.RPC) ast.Expr {
...
func (g *Generator) genMethodBody(ctx *context, rpc *protobuf2.RPC, typ *ast.FuncType) *ast.BlockStmt {
	if !isGenerated(rpc.Output) {
...
func (g *Generator) genMethodBodyAssignmentsForGeneratedOutput(ctx *context, rpc *protobuf2.RPC, msg *protobuf2.Message) (lhs []ast.Expr) {
...
func (g *Generator) genMethodBodyForGeneratedOutput(ctx *context, rpc *protobuf2.RPC, typ *ast.FuncType) *ast.BlockStmt {
...
func (g *Generator) genMethodBodyForNotGeneratedOutput(ctx *context, rpc *protobuf2.RPC, typ *ast.FuncType) *ast.BlockStmt {
	body := g.genBaseMethodBody(typ)
	methodCall := g.genMethodCall(ctx, rpc)
...
//
func (g *Generator) declMethod(ctx *context, rpc *protobuf2.RPC) ast.Decl {
...
func typeName(t protobuf2.Type) string {
	if typ, ok := t.(*protobuf2.Named); ok {
		return typ.Name
	}
	return ""
}

func isGenerated(t protobuf2.Type) bool {
	if typ, ok := t.(*protobuf2.Named); ok {
